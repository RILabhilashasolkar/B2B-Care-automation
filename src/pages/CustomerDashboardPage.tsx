...
import { Plus, Search, ChevronRight, Users, Phone, Hash, Wrench } from "lucide-react";
import { useState } from "react";

export default function CustomerDashboardPage() {
  const [search, setSearch] = useState("");
  const [searchType, setSearchType] = useState<"mobile" | "serial">("mobile");
  const [searchResult, setSearchResult] = useState<typeof mockCustomers[0] | null>(null);
  const [searched, setSearched] = useState(false);
  const [activeTab, setActiveTab] = useState<"all" | "installations">("all");
  ...
  const installationTickets = mockCustomerTickets.filter((t) => t.category === "Installation");
  const displayTickets = activeTab === "installations" ? installationTickets : mockCustomerTickets;
  ...
  return (
    <div className="space-y-3 animate-fade-in">
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={searchType === "mobile" ? "Enter 10-digit mobile number..." : "Enter product serial number..."}
      ...
      {/* Tabs for All vs Installation Tickets */}
      <div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeTab === "all" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground hover:bg-accent"
              }`}
            >
              All Tickets ({mockCustomerTickets.length})
            </button>
            <button
              onClick={() => setActiveTab("installations")}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeTab === "installations" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground hover:bg-accent"
              }`}
            >
              <Wrench className="w-3.5 h-3.5" />
              Installations ({installationTickets.length})
            </button>
          </div>
        </div>
        <div className="bg-card rounded-xl border border-border divide-y divide-border">
          {displayTickets.length === 0 ? (
            <div className="px-5 py-8 text-center text-sm text-muted-foreground">
              No installation tickets yet. Create one from My Orders → Order → Item.
            </div>
          ) : (
